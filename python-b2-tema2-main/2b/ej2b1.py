...
def read_csv_custom_separator(
    ...
    """
    Leer un CSV con separador personalizado y decimal con coma.
    Convierte la columna 'Stars' a tipo float de manera segura.
    """
    df = pd.read_csv(file_path, sep=separator, decimal=decimal)

    # Convertir la columna 'Stars' a float, ignorando valores no numéricos
    df["Stars"] = pd.to_numeric(df["Stars"].astype(str).str.replace(",", "."), errors="coerce")
