...
def test_add_animal(client):
    ...
    response = client.post(
        "/animals", json={"name": "Tigre", "species": "Panthera tigris"}
    )
...
def test_add_animal_invalid(client):
    ...
    assert "WARNING:" in logs, (
        "Debe registrarse un mensaje de nivel WARNING para errores 400"
    )
...
def test_method_not_allowed(client):
    ...
    logs = client.log_capture.get_logs()
    assert "WARNING:" in logs, (
        "Debe registrarse un mensaje de nivel WARNING para errores 405"
    )
