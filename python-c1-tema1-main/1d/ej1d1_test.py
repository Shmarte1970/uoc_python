...
def test_listar_sistemas_disponibles():
    ...
    assert all(isinstance(s, str) for s in sistemas), (
...
def test_crear_dataframe_estaciones(mock_stations):
    ...
    assert df.iloc[0]["name"] == "Station 1", (
        "El nombre de la primera estación debe ser 'Station 1'"
    )
...
def test_visualizar_estaciones(mock_savefig, mock_show, mock_stations):
    ...
    assert mock_show.called or mock_savefig.called, (
        "Debe mostrar o guardar una visualización"
    )
