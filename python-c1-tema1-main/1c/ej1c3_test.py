            ...
            "stations": [
                {
                    ...
                    "vehicle_docks_available": [
                        {"vehicle_type_ids": ["ICONIC", "BOOST"], "count": 33}
                    ],
                    "vehicle_types_available": [
                        {"vehicle_type_id": "BOOST", "count": 3},
                        {"vehicle_type_id": "ICONIC", "count": 9},
                    ],
                },
                {
                    "station_id": "2",
                    ...
                    "traffic": None,
                    "vehicle_docks_available": [
                        {"vehicle_type_ids": ["ICONIC", "BOOST"], "count": 23}
                    ],
                    "vehicle_types_available": [
                        {"vehicle_type_id": "BOOST", "count": 2},
                        {"vehicle_type_id": "ICONIC", "count": 0},
                    ],
                },
                {
                    "station_id": "9",
                    ...
                    "traffic": None,
                    "vehicle_docks_available": [
                        {"vehicle_type_ids": ["ICONIC", "BOOST"], "count": 15}
                    ],
                    "vehicle_types_available": [
...
class TestStationStatus:
    ...
    def test_enum_values(self):
        ...
        """
        assert hasattr(StationStatus, "IN_SERVICE"), (
            "StationStatus debe tener el valor IN_SERVICE"
        )
        assert hasattr(StationStatus, "MAINTENANCE"), (
            "StationStatus debe tener el valor MAINTENANCE"
        )
        assert StationStatus.IN_SERVICE.name == "IN_SERVICE", (
            "El nombre del enum debe ser IN_SERVICE"
        )
        assert StationStatus.MAINTENANCE.name == "MAINTENANCE", (
            "El nombre del enum debe ser MAINTENANCE"
        )
...
class TestVehicleType:
    ...
    def test_vehicle_type_attributes(self):
        ...
        assert vehicle_type.vehicle_type_id == "BOOST", (
            "El tipo de vehículo debe ser BOOST"
        )
...
class TestStationStatusInfo:
    ...
    def test_creation_from_api_data(self, station_data_operational):
        ...
        # Verificar estado
        assert station.status == StationStatus.IN_SERVICE, (
            "El estado debe ser IN_SERVICE"
        )
    ...
    def test_is_operational_method(
        self, station_data_operational, station_data_maintenance
    ):
        """
    ...
    def test_str_representation(self, station_data_operational):
        ...
        assert "12" in str_rep, (
            "La representación debe incluir el número de bicicletas disponibles"
        )
...
class TestBarcelonaBikingClient:
    ...
    def test_get_stations_status_success(
        self, mock_get, sample_station_status_response
    ):
        """
        ...
        mock_get.assert_called_once_with(
            "https://barcelona.publicbikesystem.net/customer/gbfs/v2/en/station_status"
        )

        # Verificar que se devolvieron las estaciones y el timestamp
        assert len(stations) == 3, "Deben devolverse 3 estaciones"
        assert all(isinstance(station, StationStatusInfo) for station in stations), (
            "Todas deben ser instancias de StationStatusInfo"
        )
        assert last_updated == 1759835019, (
            "El timestamp de actualización debe ser correcto"
        )
    ...
    def test_find_station_by_id(
        self,
        mock_get_stations_status,
        station_data_operational,
        station_data_maintenance,
    ):
        """
    ...
    @patch("ej1c3.BarcelonaBikingClient.get_stations_status")
    def test_get_operational_stations(
        self,
        mock_get_stations_status,
        station_data_operational,
        station_data_maintenance,
        ...
        assert operational[0].station_id == "1", (
            "La estación operativa debe ser la ID 1"
        )

    @patch("ej1c3.BarcelonaBikingClient.get_stations_status")
    def test_get_stations_with_available_bikes(
        self,
        mock_get_stations_status,
        station_data_operational,
        station_data_maintenance,
        ...
        assert len(with_bikes) == 1, (
            "Solo debe haber 1 estación con más de 5 bicicletas"
        )
        assert with_bikes[0].station_id == "1", (
            "La estación con bicicletas debe ser la ID 1"
        )
