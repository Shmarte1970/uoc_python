...
def test_get_stations_data_success(mock_get, sample_stations_response):
    ...
    assert result == sample_stations_response["data"], (
        "La función debe devolver el objeto 'data' de la respuesta JSON"
    )
...
def test_get_stations_data_error_status(mock_get):
    ...
    assert result is None, (
        "La función debe devolver None cuando hay un error de estado HTTP"
    )
...
def test_get_stations_data_connection_error(mock_get):
    ...
    assert result is None, (
        "La función debe devolver None cuando hay un error de conexión"
    )
...
def test_get_station_info_success(sample_stations_data):
    ...
    assert result["station_id"] == "1", (
        "La estación encontrada debe tener el ID correcto"
    )
    assert result["name"] == "GRAN VIA CORTS CATALANES, 760", (
        "La estación debe tener el nombre correcto"
    )
...
def test_get_station_info_invalid_input():
    ...
    assert get_station_info(None, "1") is None, (
        "La función debe devolver None cuando stations_data es None"
    )

    # Verificar con datos de entrada con formato incorrecto (sin estaciones)
    invalid_data = {"other_field": "value"}
    assert get_station_info(invalid_data, "1") is None, (
        "La función debe devolver None cuando stations_data no tiene la estructura esperada"
    )
...
def test_get_station_coordinates_missing_fields():
    ...
    assert get_station_coordinates(station_without_coords) is None, (
        "Debe devolver None cuando faltan coordenadas"
    )
    ...
    assert get_station_coordinates(station_without_lat) is None, (
        "Debe devolver None cuando falta la latitud"
    )
    ...
    assert get_station_coordinates(station_without_lon) is None, (
        "Debe devolver None cuando falta la longitud"
    )
...
def test_get_station_coordinates_invalid_input():
    ...
    """
    assert get_station_coordinates(None) is None, (
        "Debe devolver None cuando station_info es None"
    )
...
def test_create_stations_dataframe_success(sample_stations_data):
    ...
    assert first_station["name"] == "GRAN VIA CORTS CATALANES, 760", (
        "El nombre debe ser correcto"
    )
...
def test_create_stations_dataframe_invalid_input():
    ...
    """
    # Con datos None
    assert create_stations_dataframe(None) is None, (
        "Debe devolver None cuando stations_data es None"
    )

    # Con datos sin el formato esperado
    invalid_data = {"other_field": "value"}
    assert create_stations_dataframe(invalid_data) is None, (
        "Debe devolver None cuando los datos no tienen el formato esperado"
    )
    ...
    assert isinstance(df, pd.DataFrame), (
        "Debe devolver un DataFrame vacío cuando no hay estaciones"
    )
