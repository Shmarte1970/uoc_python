...
def test_404_client_error(mock_responses):
    ...
    assert "error_type" in result, (
        "El resultado debe contener el campo 'error_type' para errores"
    )
    assert result["error_type"] == "client_error", (
        "Para un 404, 'error_type' debe ser 'client_error'"
    )
...
def test_500_server_error(mock_responses):
    ...
    assert result["error_type"] == "server_error", (
        "Para un 500, 'error_type' debe ser 'server_error'"
    )
...
def test_301_redirect(mock_responses):
    ...
    assert result["success"] is False, (
        "Para un 301, 'success' debe ser False (es una redirección)"
    )
    ...
    assert "redirect_url" in result, (
        "El resultado debe contener el campo 'redirect_url' para redirecciones"
    )
    assert result["redirect_url"] == "https://httpstatuses.maor.io/", (
        "La URL de redirección debe ser correcta"
    )
...
def test_200_success(mock_responses):
    ...
    assert "error_type" not in result, (
        "No debe haber campo 'error_type' para respuestas exitosas"
    )
...
def test_connection_error():
    ...
    with patch("requests.get") as mock_get:
        ...
        assert isinstance(result, dict), (
            "La función debe devolver un diccionario incluso en caso de error de conexión"
        )
        ...
        # Verificación de valores para el caso específico
        assert result["success"] is False, (
            "Para un error de conexión, 'success' debe ser False"
        )
        assert "connection_error" in str(result["message"]).lower(), (
            "El mensaje debe indicar que hubo un error de conexión"
        )
