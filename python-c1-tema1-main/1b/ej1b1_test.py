...
def mock_responses():
    ...
    with responses.RequestsMock() as rsps:
        ...
        rsps.add(
            responses.GET, "https://api.ipify.org/ip", body="Not Found", status=404
        )
        yield rsps
...
def test_get_nonexistent_resource(mock_responses):
    ...
    assert "status_code" in result, (
        "El diccionario debe contener la clave 'status_code'"
    )
    assert "requested_url" in result, (
        "El diccionario debe contener la clave 'requested_url'"
    )
    assert "error_message" in result, (
        "El diccionario debe contener la clave 'error_message'"
    )

    # Verificar que los valores son correctos
    assert result["status_code"] == 404, "El código de estado debe ser 404"
    assert result["requested_url"] == "https://api.ipify.org/ip", (
        "La URL debe ser la solicitada"
    )
...
def test_get_nonexistent_resource_failure():
    ...
    with patch("requests.get") as mock_get:
        ...
        assert isinstance(result, dict), (
            "La función debe devolver un diccionario incluso en caso de error"
        )
        assert "status_code" in result, (
            "El diccionario debe contener la clave 'status_code'"
        )
        ...
        ), "El código de estado debe ser None o un número"
        assert result["requested_url"] == "https://api.ipify.org/ip", (
            "La URL debe ser la solicitada"
        )
...
def test_get_nonexistent_resource_specific_error(mock_get):
    ...
    mock_response.raise_for_status.side_effect = requests.HTTPError(
        "404 Client Error: Not Found for url: https://api.ipify.org/ip"
    )
