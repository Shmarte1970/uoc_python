...
def test_time_endpoint(server):
    ...
    assert isinstance(data["timestamp"], (int, float)), (
        "El timestamp debe ser un número."
    )
    assert isinstance(data["iso_format"], str), (
        "El formato ISO debe ser una cadena de texto."
    )
    assert isinstance(data["readable"], str), (
        "El formato legible debe ser una cadena de texto."
    )
...
def test_custom_404_error(server):
    ...
    assert response.status_code == 404, (
        "El código de estado debe ser 404 para rutas inexistentes."
    )
    assert response.headers["Content-Type"] == "application/json", (
        "El tipo de contenido del error debe ser application/json."
    )
    ...
    assert "code" in data or "status" in data, (
        "La respuesta debe incluir un campo de código de error ('code' o 'status')."
    )

    # Verificar el mensaje de error
    message_field_options = ["message", "descripcion", "detail"]
    message_field = next(
        (field for field in message_field_options if field in data), None
    )
    assert message_field is not None, (
        f"La respuesta debe incluir un campo de mensaje de error ({', '.join(message_field_options)})."
    )

    # Verificar que los valores son del tipo correcto
    code_field = "code" if "code" in data else "status"
    assert isinstance(data[code_field], int), (
        f"El campo '{code_field}' debe ser un número entero."
    )
    assert isinstance(data[message_field], str), (
        f"El campo '{message_field}' debe ser una cadena de texto."
    )

    # Verificar que el mensaje de error incluye la ruta solicitada
    assert test_path in data[message_field], (
        f"El mensaje de error debe incluir la ruta solicitada '{test_path}'."
    )
